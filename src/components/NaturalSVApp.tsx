...
function Shell({
  ...
  return (
    <div className="min-h-screen bg-[#f7f9f4]">
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-[270px] border-r border-[#dfe8db] bg-white p-5 transition-transform duration-300 ease-in-out lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="flex items-center justify-between">
          <Logo />
          <button onClick={() => setOpen(false)} className="lg:hidden text-gray-500 hover:text-gray-900">
            <X size={24} />
          </button>
        </div>
      ...
      <div className="lg:pl-[270px]">
        ...
        <main className="mx-auto max-w-[1450px] p-4 sm:p-8">{children}</main>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="animate-enter space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <Link href="/pedidos" className={`${btn} bg-[#315f3f] text-white w-full sm:w-auto`}>
          Gestionar pedidos
          <ChevronRight size={18} />
        </Link>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
        <Card className="p-5 sm:p-6 overflow-hidden">
          <div className="flex items-center justify-between">
            ...
            <Link href="/pedidos" className="text-sm font-bold text-[#4f7536] hover:underline">
              Ver todos
            </Link>
          </div>
            ...
            <table className="w-full min-w-[500px] text-left text-sm whitespace-nowrap">
              ...
              <tbody>
                {orders.slice(0, 4).map((o) => (
                  <tr
                    key={o.id}
                    className="border-b border-[#edf1eb] last:border-0 hover:bg-gray-50/50"
                  >
            ...
            {ranking.map(([name, value], index) => (
              <div key={`${name}-${index}`}>
                <div className="mb-2 flex justify-between gap-3 text-sm">
                  ...
                  <b className="text-[#315f3f]">{value} uds</b>
                </div>
                <div className="h-2.5 overflow-hidden rounded-full bg-[#eaf0e6]">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-[#4f7b37] to-[#a7c957] transition-all duration-1000"
      ...
      {low.length > 0 && (
        <Card className="border-red-200 bg-red-50 p-5 shadow-sm">
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
            ...
            <div className="flex-1">
              ...
              <p className="mt-1 text-sm text-red-800">
                Tienes {low.length} producto(s) por debajo del stock mínimo. Revisa: <span className="font-semibold">{low.map((p) => `${p.name} (${p.stock})`).join(", ")}</span>.
              </p>
            </div>
            <Link href="/productos" className="px-4 py-2 bg-white border border-red-200 text-red-700 font-medium rounded-lg text-sm hover:bg-red-50 transition w-full sm:w-auto text-center">
              Ir a productos
            </Link>
          </div>
        </Card>
...
function ProductModal({
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-gray-900/60 p-4 backdrop-blur-sm">
      <form
        onSubmit={submit}
        className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl sm:p-8 animate-enter"
      >
        <div className="flex items-start justify-between">
          <div>
          ...
          <button
            type="button"
            onClick={onClose}
            className="grid h-10 w-10 place-items-center rounded-xl bg-gray-100 hover:bg-gray-200 transition"
          >
            <X size={20} />
          </button>
        </div>
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <label className="flex items-center gap-3 pt-7 text-sm font-semibold">
            <input
              type="checkbox"
              className="h-5 w-5 accent-[#4f7536] rounded"
              checked={form.active}
        ...
        <div className="mt-6 flex flex-col-reverse sm:flex-row justify-end gap-3">
          <button
            type="button"
            onClick={onClose}
            className={`${btn} border border-[#dce4d9] hover:bg-gray-50`}
          >
            Cancelar
          </button>
          <button className={`${btn} bg-[#315f3f] text-white hover:bg-[#25472e]`}>
            Guardar producto
          </button>
        </div>
      </form>
    ...
    <div className="animate-enter space-y-5">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <button
          ...
          className={`${btn} bg-[#315f3f] text-white w-full sm:w-auto`}
        >
          <PackagePlus size={19} />
          Agregar producto
        </button>
      ...
      <Card>
        ...
        <div className="overflow-x-auto w-full">
          <table className="w-full min-w-[820px] text-left text-sm whitespace-nowrap">
            ...
            <tbody>
              {shown.map((p) => (
                <tr key={p.id} className="border-t border-[#e8ede5] hover:bg-gray-50/50">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[#eef3ea] text-2xl overflow-hidden">
                        {p.imageUrl ? (
                          <img 
                            src={p.imageUrl} 
                            alt={p.name} 
                      ...
                      <div>
                        ...
                        <p className="max-w-[200px] truncate text-xs text-[#7b877f]">
                          {p.description}
                        </p>
                      </div>
                    </div>
                  ...
                  <td className="px-5 py-4">
                    ...
                    <span className="text-[#89938d]"> / alerta {p.minStock}</span>
                  </td>
                  <td className="px-5 py-4">
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-bold ${p.active ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-gray-100 text-gray-600 border border-gray-200"}`}
                    >
                  ...
                  <td className="px-5 py-4">
                    <div className="flex justify-end gap-2">
                      <button
                        ...
                        className="grid h-9 w-9 place-items-center rounded-lg bg-[#edf3e9] text-[#4d7336] hover:bg-[#dbe7d5] transition"
                      >
                        <Edit3 size={17} />
                      </button>
                      <button
                        ...
                        className="grid h-9 w-9 place-items-center rounded-lg bg-red-50 text-red-600 hover:bg-red-100 transition"
                      >
                        <Trash2 size={17} />
                      </button>
                    </div>
...
function OrderDetail({
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-gray-900/60 p-4 backdrop-blur-sm">
      <Card className="w-full max-w-lg p-6 sm:p-8 animate-enter">
        <div className="flex items-start justify-between">
          <div>
            <p className="text-sm font-bold text-[#708167]">
              DETALLE DEL PEDIDO
            </p>
            <h3 className="mt-1 text-xl sm:text-2xl font-bold truncate pr-4">{order.id}</h3>
          </div>
          <button
            onClick={onClose}
            className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-gray-100 hover:bg-gray-200 transition"
          >
            <X size={20} />
          </button>
        </div>
        <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-4 rounded-2xl bg-[#f4f7f1] p-4 text-sm">
          <div>
        ...
        <div className="mt-5 space-y-3 max-h-48 overflow-y-auto pr-2">
          {order.items.map((i) => (
            ...
            >
              <div>
                <b className="block">{i.name}</b>
                <p className="text-[#7d8880]">
                  {i.quantity} × {money(i.price)}
                </p>
              </div>
              <b className="self-center">{money(i.quantity * i.price)}</b>
            </div>
          ))}
        </div>
        <div className="mt-5 space-y-2 text-sm bg-gray-50 p-4 rounded-xl">
          <div className="flex justify-between">
            <span className="text-gray-600">Subtotal</span>
            <b className="text-gray-900">{money(order.subtotal)}</b>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600">Envío</span>
            <b className="text-gray-900">{order.shipping ? money(order.shipping) : "Gratis"}</b>
          </div>
          <div className="flex justify-between border-t border-gray-200 pt-3 text-lg">
            <b className="text-gray-900">Total</b>
            <b className="text-[#315f3f]">{money(order.total)}</b>
          </div>
        </div>
      </Card>
    </div>
...
function Orders({ mine = false }: { mine?: boolean }) {
  ...
  return (
    <div className="animate-enter space-y-5">
      ...
      <Card>
        <div className="flex flex-col gap-4 border-b border-[#e3e9e0] p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex gap-2 overflow-x-auto pb-2 sm:pb-0 scrollbar-hide">
            {[
              ...
              <button
                key={s}
                onClick={() => setStatus(s)}
                className={`whitespace-nowrap rounded-lg px-4 py-2 text-sm font-semibold transition ${status === s ? "bg-[#315f3f] text-white" : "bg-[#f1f4ef] text-[#69766d] hover:bg-gray-200"}`}
              >
                {s}
              </button>
            ))}
          </div>
          <p className="text-sm text-[#758078] whitespace-nowrap">{shown.length} resultados</p>
        </div>
        <div className="overflow-x-auto w-full">
          <table className="w-full min-w-[850px] text-left text-sm whitespace-nowrap">
            <thead className="bg-[#f8faf7] text-[#6f7b73] border-b">
              <tr>
            ...
            <tbody>
              {shown.map((o) => (
                <tr key={o.id} className="border-t border-[#e7ede4] hover:bg-gray-50/50">
                  ...
                  <td className="px-5 py-4">
                    {mine ? (
                      ...
                      <select
                        ...
                        className={`rounded-lg border px-2.5 py-2 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-gray-300 ${statusColor[o.status]}`}
                      >
                        {[
                          "Pendiente",
                          "Confirmado",
                  ...
                  <td className="px-5 py-4 text-right">
                    <button
                      onClick={() => setDetail(o)}
                      className="inline-grid h-9 w-9 place-items-center rounded-lg bg-[#edf3e9] text-[#4c7137] hover:bg-[#d5e0d0] transition"
                    >
                      <Eye size={17} />
                    </button>
                  </td>
    ...
    <div className="animate-enter space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[.95fr_1.05fr]">
        <Card className="flex flex-col h-full max-h-[700px]">
          <div className="border-b border-[#e4eae1] p-4 shrink-0">
          ...
          <div className="overflow-y-auto p-2 flex-1">
            {clients.map((c) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-xl p-3 text-left transition ${selected?.id === c.id ? "bg-[#edf4e9] ring-1 ring-[#c0d4b5]" : "hover:bg-[#f6f8f4]"}`}
                >
                  ...
                  <span className="text-xs font-bold text-[#6e7d71] whitespace-nowrap bg-gray-100 px-2 py-1 rounded-md">
            ...
            {!clients.length && (
              <p className="p-4 text-center text-sm text-gray-500">No se encontraron clientes.</p>
            )}
          </div>
        </Card>
        <Card className="h-full">
          {selected ? (
              ...
              return (
                <div className="p-5 sm:p-6 h-full flex flex-col">
                  <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 text-center sm:text-left">
                    <div className="grid h-16 w-16 place-items-center rounded-2xl bg-[#eaf2e6] text-2xl font-bold text-[#3f6831]">
                      {selected.name[0]}
                    </div>
                    <div>
                      <h3 className="text-xl sm:text-2xl font-bold text-gray-900">{selected.name}</h3>
                      <p className="text-sm text-[#77847a] mt-1">
                        {selected.phone || "Sin teléfono"} · {selected.email}
                      </p>
                    </div>
                  </div>
                  <div className="mt-6 grid grid-cols-2 gap-3 shrink-0">
                    <div className="rounded-xl bg-[#f3f7f0] p-4 text-center sm:text-left">
                      <p className="text-sm text-[#748078]">Total Pedidos</p>
                      <b className="text-2xl text-gray-900">{history.length}</b>
                    </div>
                    <div className="rounded-xl bg-[#f3f7f0] p-4 text-center sm:text-left">
                      <p className="text-sm text-[#748078]">
                        Compras entregadas
                      </p>
                      <b className="text-2xl text-[#315f3f]">{money(spent)}</b>
                    </div>
                  </div>
                  <h4 className="mt-7 font-bold text-gray-800 shrink-0">Historial de compras</h4>
                  <div className="mt-4 space-y-3 overflow-y-auto flex-1 pr-1">
                    {history.map((o) => (
                      <div
                        key={o.id}
                        className="flex items-center justify-between rounded-xl border border-[#e2e9df] p-4 hover:shadow-sm transition"
                      >
                        <div className="min-w-0 pr-2">
                          <b className="truncate block text-gray-900">{o.id}</b>
                          <p className="text-xs text-[#7b877f] mt-1">
                            {o.date} · <span className="font-medium">{o.items.length} productos</span>
                          </p>
                        </div>
                        <div className="text-right shrink-0">
                          <b className="text-gray-900">{money(o.total)}</b>
                          <div className="mt-1">
                            <Status status={o.status} />
                          </div>
                        </div>
                    ...
                    {!history.length && (
                      <div className="text-center py-8">
                        <p className="text-sm text-[#7b877f]">
                          Este cliente aún no tiene compras finalizadas.
                        </p>
                      </div>
                    )}
                  </div>
            ...
            <Empty
              ...
              text="Consulta sus datos y todo su historial de compras detallado."
            />
          )}
        </Card>
      </div>
      ...
      <Card className="overflow-hidden">
        <div className="overflow-x-auto w-full">
          <table className="w-full min-w-[600px] text-left text-sm whitespace-nowrap">
            <thead className="bg-[#f6f9f4] text-[#6e7b72] border-b">
              <tr>
                <th className="px-5 py-3 font-semibold">Usuario</th>
                <th className="px-5 py-3 font-semibold">Rol</th>
                <th className="px-5 py-3 font-semibold hidden sm:table-cell">Teléfono</th>
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id} className="border-t border-[#e6ece3] hover:bg-gray-50/50 transition">
                  <td className="px-5 py-4">
                    <b className="text-gray-900 block truncate max-w-[150px] sm:max-w-xs">{u.name}</b>
                    <p className="text-xs text-[#7b877f] truncate max-w-[150px] sm:max-w-xs">{u.email}</p>
                  </td>
                  <td className="px-5 py-4">
                    <span className="rounded-full bg-[#edf3e9] px-2.5 py-1 text-xs font-bold capitalize text-[#4c7138] border border-[#d2e0ca]">
                      {u.role === "admin" ? "Administrador" : "Cliente"}
                    </span>
                  </td>
                  <td className="px-5 py-4 hidden sm:table-cell text-gray-600">{u.phone || "—"}</td>
                  <td className="px-5 py-4">
                    <button
                      onClick={() => toggle(u)}
                      className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold transition-colors border ${
                        u.active 
                        ? "bg-red-50 text-red-700 hover:bg-red-100 border-red-200" 
                        : "bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border-emerald-200"
                      }`}
                    >
                      {u.active ? <><UserX size={14}/> Bloquear</> : <><UserCheck size={14}/> Activar</>}
                    </button>
                  </td>
                  <td className="px-5 py-4">
                    <div className="flex justify-end gap-2">
                      <button
                        ...
                        className="grid h-9 w-9 place-items-center rounded-lg bg-[#edf3e9] text-[#4d7336] hover:bg-[#d2e0ca] transition"
                      >
                        <Edit3 size={17} />
                      </button>
                      <button
...
function UserModal({ user, onClose }: { user: User; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-gray-900/60 p-4 backdrop-blur-sm">
      <form
        onSubmit={submit}
        className="w-full max-w-lg rounded-3xl bg-white p-6 sm:p-8 animate-enter"
      >
        <div className="flex justify-between items-start mb-5">
          <h3 className="text-2xl font-bold text-gray-900">Editar usuario</h3>
          <button type="button" onClick={onClose} className="p-2 bg-gray-100 hover:bg-gray-200 rounded-xl transition">
            <X size={20}/>
          </button>
        </div>
        ...
        <div className="mt-8 flex flex-col-reverse sm:flex-row justify-end gap-3">
          <button type="button" onClick={onClose} className={`${btn} border border-gray-300 hover:bg-gray-50`}>
            Cancelar
          </button>
          <button className={`${btn} bg-[#315f3f] text-white hover:bg-[#22452e]`}>Guardar cambios</button>
        </div>
      </form>
    </div>
  );
    ...
    <div className="animate-enter space-y-6">
      <section className="relative overflow-hidden rounded-[28px] bg-[#294f37] p-6 text-white soft-grid sm:p-9">
        <div className="relative max-w-2xl">
          ...
          <label className="relative mt-6 block w-full sm:max-w-lg text-[#18251d]">
            <Search
              className="absolute left-4 top-3.5 text-[#718077]"
              size={20}
            />
            <input
              className="w-full rounded-xl bg-white py-3.5 pl-11 pr-4 outline-none focus:ring-2 focus:ring-[#8fad55]"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <Link
          href="/carrito"
          className="flex items-center gap-2 text-sm font-bold text-[#4d7336] bg-[#edf4e9] px-4 py-2 rounded-xl hover:bg-[#dce8d2] transition w-full sm:w-auto justify-center"
        >
      ...
      <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
        {active.map((p) => (
          <Card key={p.id} className="group overflow-hidden flex flex-col">
            <div className="grid h-48 place-items-center bg-gradient-to-br from-[#eff4e9] to-[#dce8d2] text-7xl transition-transform duration-500 group-hover:scale-[1.02]">
              {p.imageUrl ? (
            ...
            <div className="p-5 flex flex-col flex-1">
              ...
              <h4 className="mt-2 text-xl font-bold text-gray-900">{p.name}</h4>
              <p className="mt-2 text-sm leading-relaxed text-[#6e7b73] flex-1">
                {p.description}
              </p>
        ...
        {!active.length && (
          <div className="col-span-full">
            <Empty icon={Search} title="Sin resultados" text="No encontramos productos con esa búsqueda." />
          </div>
        )}
      </div>
    </div>
...
function Cart() {
  ...
  const cartDetails = cart
    ...
    .filter(
      (item): item is Product & { quantity: number } => item !== null
    );
    ...
    <div className="animate-enter space-y-5">
      ...
      {!cart.length ? (
        ...
        <div className="grid gap-6 xl:grid-cols-[1fr_390px]">
          <Card className="p-4 sm:p-6">
            <div className="space-y-4">
              {cartDetails.map((i) => (
                <div
                  key={i.id}
                  className="flex flex-col gap-4 rounded-2xl border border-[#e1e8de] p-4 sm:flex-row sm:items-center relative"
                >
                  <div className="grid h-16 w-16 shrink-0 place-items-center rounded-xl bg-[#edf3e8] text-3xl overflow-hidden">
                  ...
                  <div className="min-w-0 flex-1 pr-8 sm:pr-0">
                    ...
                    <p className="text-sm text-[#77847a]">
                      {money(i.price)} c/u · <span className="font-medium">{i.stock} en stock</span>
                    </p>
                  </div>

                  <button
                    onClick={() => removeFromCart(i.id)}
                    className="text-red-500 hover:text-red-700 bg-red-50 p-2 rounded-lg sm:bg-transparent sm:p-0 absolute top-4 right-4 sm:relative sm:top-0 sm:right-0 transition"
                    aria-label="Eliminar"
                  >
                    <Trash2 size={18} />
                  </button>
                  <div className="flex items-center justify-between sm:justify-end gap-4 mt-2 sm:mt-0 w-full sm:w-auto">
                    <div className="flex items-center gap-2 bg-gray-50 rounded-lg p-1 border">
                      <button
                        ...
                        className="grid h-8 w-8 place-items-center rounded-md bg-white border shadow-sm hover:bg-gray-100"
                      >
                        <Minus size={16} />
                      </button>
                      ...
                      <button
                        ...
                        onClick={() => changeCartQuantity(i.id, i.quantity + 1)}
                        className="grid h-8 w-8 place-items-center rounded-md bg-white border shadow-sm disabled:cursor-not-allowed disabled:opacity-40 hover:bg-gray-100"
                      >
                        <Plus size={16} />
                      </button>
                    </div>
                    <b className="w-24 text-right text-lg text-[#315f3f]">
          ...
          <Card className="h-fit p-5 sm:p-6 sticky top-24">
            <h3 className="text-xl font-bold border-b border-gray-100 pb-4">Resumen de compra</h3>
            <label className="mt-5 block text-sm font-semibold">
              ...
              <textarea
                ...
                placeholder="Ingresa municipio, departamento y calle..."
              />
            </label>
            <div className="mt-6 space-y-3 text-sm bg-gray-50 p-4 rounded-xl">
              <div className="flex justify-between text-gray-600">
                <span>Subtotal</span>
                <b className="text-gray-900">{money(subtotal)}</b>
              </div>
              <div className="flex justify-between text-gray-600">
                <span>Envío</span>
                <b className="text-gray-900">{shipping ? money(shipping) : "Gratis"}</b>
              </div>
              <div className="flex justify-between border-t border-gray-200 pt-4 text-xl">
                <b className="text-gray-900">Total</b>
                <b className="text-[#315f3f]">{money(total)}</b>
              </div>
            </div>
            {subtotal < 45 && (
              <p className="mt-4 text-xs text-center text-amber-700 bg-amber-50 p-2 rounded-lg border border-amber-100">
                Faltan <b>{money(45 - subtotal)}</b> para obtener <b>envío gratis</b>.
              </p>
            )}
            <button
              onClick={checkout}
              className={`${btn} mt-5 w-full bg-[#315f3f] py-4 text-white text-lg hover:bg-[#25492f] shadow-lg`}
            >
              Confirmar y Pagar
...
function Profile() {
  ...
  return (
    <div className="mx-auto max-w-2xl animate-enter">
      <Card className="overflow-hidden">
        <div className="h-32 bg-[#294f37] soft-grid" />
        <div className="p-6 sm:p-8">
          <div className="-mt-16 grid h-24 w-24 place-items-center rounded-2xl border-4 border-white bg-[#a7c957] text-3xl font-bold text-[#213c2b] shadow-md">
            {currentUser?.name
              .split(" ")
              .map((n) => n[0])
              .slice(0, 2)
          ...
          <h2 className="mt-5 text-2xl sm:text-3xl font-bold text-gray-900">{currentUser?.name}</h2>
          <p className="text-[#748078] capitalize">{currentUser?.role === 'admin' ? 'Administrador del sistema' : 'Cuenta de cliente'}</p>
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            <div className="rounded-xl bg-[#f3f6f0] p-5 border border-[#e1e8dc]">
              <p className="text-xs font-bold uppercase tracking-wide text-[#7b887e]">
                Correo Electrónico
              </p>
              <p className="mt-2 font-semibold text-gray-900 break-all">{currentUser?.email}</p>
            </div>
            <div className="rounded-xl bg-[#f3f6f0] p-5 border border-[#e1e8dc]">
              <p className="text-xs font-bold uppercase tracking-wide text-[#7b887e]">
